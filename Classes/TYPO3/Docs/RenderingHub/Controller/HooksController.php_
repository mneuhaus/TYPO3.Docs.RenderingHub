...
class HooksController extends \TYPO3\Flow\Mvc\Controller\ActionController {
	...
	public function indexAction($type = NULL) {
		...
		$handlerObjectName = $this->objectManager->getCaseSensitiveObjectName('TYPO3\Docs\RenderingHub\Hooks\\' . $type . 'Handler');
		if ($this->objectManager->isRegistered($handlerObjectName)) {
			/**
			 * @var \TYPO3\Docs\RenderingHub\Hooks\HookHandlerInterface $handler
			 */
			$handler = $this->objectManager->get($handlerObjectName);
			$packages = $handler->getPackages($this->request);

			$documentServiceClassName = 'TYPO3\Docs\RenderingHub\Service\Document\\' . ucfirst($handler->getRepositoryType()) . 'Service';
	 ...
	 * @return void
	 */
	public function queueForRendering(\TYPO3\Docs\RenderingHub\Domain\Model\Document $document) {
	}
