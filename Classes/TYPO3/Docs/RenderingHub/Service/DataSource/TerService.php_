<?php
namespace TYPO3\Docs\RenderingHub\Service\DataSource;
 ...
 */
class TerService implements \TYPO3\Docs\RenderingHub\Service\DataSource\ServiceInterface {
	...
	public function get() {
		...
		if (!is_file($this->settings['terDatasource'])) {
			throw new \TYPO3\Docs\RenderingHub\Exception\MissingDataSourceException('There is no data source. File not found ' . $this->settings['terDatasource'], 1345549138);
		}
		...
		if (!is_object($entries)) {
			throw new \TYPO3\Docs\RenderingHub\Exception\XmlParsingException('Error while parsing ' . $this->settings['terDatasource'], 1300783708);
		}
	...
	protected function isCacheObsolete() {
		$localUnixTime = \TYPO3\Docs\RenderingHub\Utility\Files::getModificationTime($this->settings['terDatasource']);
		$remoteUnixTime = \TYPO3\Docs\RenderingHub\Utility\Files::getRemoteModificationTime($this->settings['terDatasourceRemote']);
	...
	public function write() {
		...
		\TYPO3\Docs\RenderingHub\Utility\Files::write($this->settings['terDatasource'], $content);
	}
}
