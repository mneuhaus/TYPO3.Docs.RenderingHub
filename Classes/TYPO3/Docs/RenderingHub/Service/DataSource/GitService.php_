 ...
 */
class GitService implements \TYPO3\Docs\RenderingHub\Service\DataSource\ServiceInterface {
	...
	/**
	 * @Flow\Inject
	 * @var \TYPO3\Docs\RenderingHub\Finder\Repository
	 */
	...
	public function get() {
		...
		if (!is_file($this->settings['gitDatasource'])) {
			throw new \TYPO3\Docs\RenderingHub\Exception\MissingDataSourceException('There is no data source. File not found ' . $this->settings['gitDatasource'], 1345549239);
		}
	...
	protected function isCacheObsolete() {
		$fileModificationTime = \TYPO3\Docs\RenderingHub\Utility\Files::getModificationTime($this->settings['gitDatasource']);
	...
	public function write() {
		...
		// write cache
		\TYPO3\Docs\RenderingHub\Utility\Files::write($this->settings['gitDatasource'], json_encode($cachePackages));
	}
