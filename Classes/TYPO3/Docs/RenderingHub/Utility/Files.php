...
class Files  extends \TYPO3\Flow\Utility\Files {
	...
	static public function getRemoteModificationTime($uri) {
		$handle = fopen($uri, "r");
		if (!$handle) {
			throw new \TYPO3\Docs\RenderingHub\Exception\MissingFileException('Data source was not found at ' . $uri, 1345213889);
		}
	 ...
	 *
	 * @throws \TYPO3\Docs\RenderingHub\Exception\WriteFileException
	 * @param string $file
	...
	static public function write($file, $content) {
		...
		if (!$result) {
			throw new \TYPO3\Docs\RenderingHub\Exception\WriteFileException('Exception thrown #1300100506: not possible to write file at "' . $file . '"', 1300100506);
		}
