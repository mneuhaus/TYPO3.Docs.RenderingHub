...
class SystemLogger {
	...
	public function log($message, $severity = LOG_INFO, $additionalData = NULL) {
		...
		switch ($severity) {
			case LOG_INFO:
				...
				break;
			case LOG_WARNING:
				\TYPO3\Docs\Utility\Console::output('WARNING:' . $message);
				if ($this->settings['sendEmailForWarningMessage']) {
					$this->email($message, 'Warning');
				}
				break;
			case LOG_ALERT:
				\TYPO3\Docs\Utility\Console::output('ALERT:' . $message);
	...
	protected function email($message, $severity) {
		...
		if (!$this->message->isSent()) {
			throw new \TYPO3\Docs\Exception\InvalidConfigurationException('No email has been sent. Check Swift Mailer configuration', 1345829105);
		}
